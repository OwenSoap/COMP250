 ...
 *   
 *   Discussed the assignment depth with Boqi Chen
...
public class NaturalNumber {
	...
	public NaturalNumber plus(NaturalNumber second) throws Exception {
		...
		int carry = 0; // Define the carry as an integer
		int singleResult = 0; // Define the single result as an integer

		// A for loop for calculation
		for (int i = 0; i <= firstClone.coefficients.size() - 1; i++) {

			singleResult = (firstClone.coefficients.get(i) + secondClone.coefficients.get(i) + carry) % base;
			carry = (firstClone.coefficients.get(i) + secondClone.coefficients.get(i) + carry) / base;
			sum.coefficients.add(singleResult);

		}

		// Add carry to the first digit if it is greater than zero
		if (carry > 0) {
	...
	public NaturalNumber times(NaturalNumber multiplicand) throws Exception {
		...
		NaturalNumber zero = new NaturalNumber(0, this.base); // A zero natural
																// number

		// Check whether there is a 0 in multiplicand or multiplier
		if (multiplicand.compareTo(zero) == 0 || this.compareTo(zero) == 0) {
			return zero;
		}

		int singleDigit = 0; // Define the single digit as an integer
		NaturalNumber firstResult; // A natural number firstResult to store the
									// first result of production

		// For loop to add multiple single product results
		for (int i = 0; i <= this.coefficients.size() - 1; i++) {
			singleDigit = this.coefficients.get(i);
			firstResult = multiplicand.timesSingleDigit(singleDigit);
			for (int j = 0; j < i; j++) {
				firstResult.coefficients.addFirst(0);
			}
			product = product.plus(firstResult);
		}
	...
	public NaturalNumber timesSingleDigit(int digit) {

		// Define a natural number to store single product
		NaturalNumber singleProduct = new NaturalNumber(this.base);
		int carry = 0; // Define an integer to store carry
		int r = 0; // Define an integer to store the result of digit production

		// Calculating single digit production
		for (int i = 0; i <= this.coefficients.size() - 1; i++) {
			r = (this.coefficients.get(i) * digit) + carry;
			carry = r / this.base;
			r = r - carry * this.base;
			singleProduct.coefficients.addLast(r);
		}

		// Add carry to the first digit if it is greater than zero
		if (carry > 0) {
			singleProduct.coefficients.addLast(carry);
		}
	...
	public NaturalNumber minus(NaturalNumber second) throws Exception {
		...
		// Rearrange two numbers to have the same digits
		...
		int borrow = 0; // Define an integer borrow
		int r = 0; // Define an integer r to store the result
		// For loop to calculate the minus operation
		for (int i = 0; i <= firstClone.coefficients.size() - 1; i++) {
			if (i < secondClone.coefficients.size()) {
				int a = firstClone.coefficients.get(i) - borrow;
				borrow = 0;
				int b = secondClone.coefficients.get(i);
				if (a < b) {
					a = a + this.base;
					borrow += 1;
				}
				r = a - b;
				difference.coefficients.addLast(r);
			}
		}
	...
	public NaturalNumber divide(NaturalNumber divisor) throws Exception {
		...
		// Define a natural number zero
		NaturalNumber zero = new NaturalNumber(0, this.base);
		// Check whether divisor is larger than dividend
		if (this.compareTo(divisor) == -1) {
			return zero;
		}

		NaturalNumber rem = new NaturalNumber(this.base); // A natural number rem to store the
								  // remainder
		boolean found = false; // Check whether found the correct remainder for the first time
		int i = this.coefficients.size() - 1; // Index starts from the highest one
		// Using a while loop for selection
		while (i >= 0 || rem.compareTo(divisor) >= 0) {
			
			// If the remainder is zero while the first correct is not found,
			// get off the zero
			if (rem.compareTo(zero) == 0 && !found) {
				rem = new NaturalNumber(this.base);
			}
			if (i >= 0) {
				rem.coefficients.addFirst(this.coefficients.get(i));
			}
			
			// Once if the reminder is larger than the divisor for the first
			// time, let the found to be true
			if (rem.compareTo(divisor) >= 0 && !found) {
				found = true;
			}
			
			// If the correct reminder has been found for the first time while
			// the digit of next reminder is less
			// than the divisor, add zero to the result
			if (rem.compareTo(divisor) < 0 && found) {
			...
			// If already found the correct reminder for the first time then calculate the result
			if (found && rem.compareTo(divisor) >= 0) {
				for (int j = 0; j < this.base; j++) {
					NaturalNumber thisResult = divisor.timesSingleDigit(j);
					if (thisResult.compareTo(rem) <= 0 && divisor.timesSingleDigit(j + 1).compareTo(rem) > 0) {
						quotient.coefficients.addFirst(j);
						rem = rem.minus(thisResult);
						// Reset the reminder if reminder is zero
						if (rem.compareTo(zero) == 0) {
							rem = new NaturalNumber(this.base);
						}
						break;
